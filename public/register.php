...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    flash('ok','Registrasi berhasil. No akun Anda: '.$accountNo.'. Simpan nomor ini untuk login setelah disetujui superadmin.'); redirect(app_url('register.php'));
}
render_header('Registrasi Mitra','auth'); ?>
<div class="auth-card card register-card"><a href="<?=app_url('index.php')?>">← Kembali ke beranda</a><span class="eyebrow">Pendaftaran Mitra</span><h1>Ajukan akun AppsBilling</h1><p>Isi data usaha dengan rapi. Setelah dikirim, akun masuk antrean review superadmin. Kalau disetujui, sistem otomatis membuat No Akun 4 digit dan ruang billing terpisah.</p><?php if($m=flash('ok')):?><div class="alert alert-ok"><?=e($m)?></div><?php endif;?><?php if($m=flash('err')):?><div class="alert alert-danger"><?=e($m)?></div><?php endif;?>
<div class="guide-steps"><div><b>1</b><span>Daftar</span><small>Isi nama usaha, PIC, dan login admin.</small></div><div><b>2</b><span>Review</span><small>Superadmin mengecek dan approve akun.</small></div><div><b>3</b><span>Aktif</span><small>Mitra login memakai No Akun + username.</small></div></div>
